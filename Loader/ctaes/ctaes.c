...
static void SubBytes(AES_state* s, int inv) {
    ...
    M46 = M44 & T6; M47 = M40 & T8; M48 = M39 & D; M49 = M43 & T16;
    M50 = M38 & T9; M51 = M37 & T17; M52 = M42 & T15; M53 = M45 & T27;
    M54 = M41 & T10; M55 = M44 & T13; M56 = M40 & T23; M57 = M39 & T19;
    M58 = M43 & T3; M59 = M38 & T22; M60 = M37 & T20; M61 = M42 & T1;
    M62 = M45 & T4; M63 = M41 & T2;

    if (inv) {
        uint16_t P0 = M52 ^ M61, P1 = M58 ^ M59, P2 = M54 ^ M62, P3 = M47 ^ M50;
        uint16_t P4 = M48 ^ M56, P5 = M46 ^ M51, P6 = M49 ^ M60, P7 = P0 ^ P1;
        uint16_t P8 = M50 ^ M53, P9 = M55 ^ M63, P10 = M57 ^ P4, P11 = P0 ^ P3;
        uint16_t P12 = M46 ^ M48, P13 = M49 ^ M51, P14 = M49 ^ M62, P15 = M54 ^ M59;
        uint16_t P16 = M57 ^ M61, P17 = M58 ^ P2, P18 = M63 ^ P5, P19 = P2 ^ P3;
        uint16_t P20 = P4 ^ P6, P22 = P2 ^ P7, P23 = P7 ^ P8, P24 = P5 ^ P7;
        uint16_t P25 = P6 ^ P10, P26 = P9 ^ P11, P27 = P10 ^ P18, P28 = P11 ^ P25;
        ...
        uint16_t L0 = M61 ^ M62, L1 = M50 ^ M56, L2 = M46 ^ M48, L3 = M47 ^ M55;
        uint16_t L4 = M54 ^ M58, L5 = M49 ^ M61, L6 = M62 ^ L5, L7 = M46 ^ L3;
        uint16_t L8 = M51 ^ M59, L9 = M52 ^ M53, L10 = M53 ^ L4, L11 = M60 ^ L2;
        uint16_t L12 = M48 ^ M51, L13 = M50 ^ L0, L14 = M52 ^ M61, L15 = M55 ^ L1;
        uint16_t L16 = M56 ^ L0, L17 = M57 ^ L1, L18 = M58 ^ L8, L19 = M63 ^ L4;
        uint16_t L20 = L0 ^ L1, L21 = L1 ^ L7, L22 = L3 ^ L12, L23 = L18 ^ L2;
        uint16_t L24 = L15 ^ L9, L25 = L6 ^ L10, L26 = L7 ^ L9, L27 = L8 ^ L10;
        uint16_t L28 = L11 ^ L14, L29 = L11 ^ L17;
...
static void GetOneColumn(AES_state* s, const AES_state* a, int c) {
    int b;
    for (b = 0; b < 8; b++) s->slice[b] = (a->slice[b] >> c) & 0x1111;
}
...
static void MultX(AES_state* s) {
    ...
    s->slice[7] = s->slice[6]; s->slice[6] = s->slice[5]; s->slice[5] = s->slice[4];
    s->slice[4] = s->slice[3] ^ top; s->slice[3] = s->slice[2] ^ top;
    s->slice[2] = s->slice[1]; s->slice[1] = s->slice[0] ^ top; s->slice[0] = top;
}
...
static void AES_setup(AES_state* rounds, const uint8_t* key, int nkeywords, int nrounds) {
    ...
    for (i = 0; i < nrounds + 1; i++) {
        int b;
        for (b = 0; b < 8; b++) rounds[i].slice[b] = 0;
    }
    for (i = 0; i < nkeywords; i++) {
        int r;
        for (r = 0; r < 4; r++) LoadByte(&rounds[i >> 2], *(key++), r, i & 3);
    }
...
static void AES_encrypt(const AES_state* rounds, int nrounds, unsigned char* cipher16, const unsigned char* plain16) {
    ...
    for (round = 1; round < nrounds; round++) {
        SubBytes(&s, 0); ShiftRows(&s); MixColumns(&s, 0); AddRoundKey(&s, rounds++);
    }
    SubBytes(&s, 0); ShiftRows(&s); AddRoundKey(&s, rounds);
...
static void AES_decrypt(const AES_state* rounds, int nrounds, unsigned char* plain16, const unsigned char* cipher16) {
    ...
    AddRoundKey(&s, rounds--);
    for (round = 1; round < nrounds; round++) {
        InvShiftRows(&s); SubBytes(&s, 1); AddRoundKey(&s, rounds--); MixColumns(&s, 1);
    }
    InvShiftRows(&s); SubBytes(&s, 1); AddRoundKey(&s, rounds);
    SaveBytes(plain16, &s);
}
...
void AES256_encrypt(const AES256_ctx* ctx, size_t blocks, unsigned char* cipher16, const unsigned char* plain16) {
    while (blocks--) { AES_encrypt(ctx->rk, 14, cipher16, plain16); cipher16 += 16; plain16 += 16; }
}

void AES256_decrypt(const AES256_ctx* ctx, size_t blocks, unsigned char* plain16, const unsigned char* cipher16) {
    while (blocks--) { AES_decrypt(ctx->rk, 14, plain16, cipher16); cipher16 += 16; plain16 += 16; }
}

static void Xor128(uint8_t* buf1, const uint8_t* buf2) {
    size_t i;
    for (i = 0; i < 16; i++) buf1[i] ^= buf2[i];
}

void AES256_CBC_init(AES256_CBC_ctx* ctx, const unsigned char* key32, const uint8_t* iv) {
    AES256_init(&(ctx->ctx), key32);
    memcpy(ctx->iv, iv, 16);
}
...
void AES256_CBC_encrypt(AES256_CBC_ctx* ctx, size_t blocks, unsigned char* encrypted, const unsigned char* plain) {
    ...
    for (i = 0; i < blocks; i++) {
        memcpy(buf, plain, 16);
        Xor128(buf, ctx->iv);
        AES_encrypt(ctx->ctx.rk, 14, encrypted, buf);
        memcpy(ctx->iv, encrypted, 16);
        plain += 16; encrypted += 16;
    }
}
...
void AES256_CBC_decrypt(AES256_CBC_ctx* ctx, size_t blocks, unsigned char* plain, const unsigned char* encrypted) {
    ...
    for (i = 0; i < blocks; i++) {
        ...
        AES_decrypt(ctx->ctx.rk, 14, plain, encrypted);
        Xor128(plain, ctx->iv);
